/*select matricula.id_matricula, aprendiz.nome from matricula
right join aprendiz on matricula.id_aprendiz = aprendiz.id_aprendiz;*/

/*  O comando RIGHT JOIN verifica duas tabelas em que as mesma possuam um dado com o mesmo valor mas tambem  mostra
    todos os dados da tabela da direita mesmo que a tabela da esquerda não tenha dados referentes a tabela da direita */
    
/*O comando abaixo mostra o nome de cada instrutor referenciando o carro que cada um possui usando a coluna "id_instrutor"
mas ainda assim mostra os demais instrutores mesmo que estes não tenham um carro*/
...
UPDATE Alunos SET Id_Curso =5 WHERE Id = 3;
...
UPDATE Alunos SET Id_Curso =4 WHERE Id = 5;
 ...
 update Alunos set Id_Curso = 2 where Id=5;
...
use revisao;
 ...
 /*ANY // mostrar aluno com  mesmo endereco que os professores*/
 select Nome,Id_Endereco from alunos where Id_endereco =  any(select Id_endereco from Professores );
 
  /*ANY // mostrar nome do aluhno curso endeereco co,mpleto em que endereco do alunos seja mesmo que os prefessores onde ID <3*/
 select Alunos.Nome,Cursos.Nome,Enderecos.* from alunos
 inner join Enderecos on Enderecos.Id= Alunos.Id_Endereco
 inner join Cursos on Cursos.Id= Alunos.Id_Curso
 where Id_endereco =  any(select Id_endereco from Professores where Id_Endereco <3);
